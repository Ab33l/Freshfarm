  ...
  <?php 
   if($_POST){
    ...
    if(empty($_POST['email']) || empty($_POST['password'])){
      $errors[] = "You must provide your email and password to proceed";
    }

    //validate email
    if(!filter_var($email,FILTER_VALIDATE_EMAIL)){
      $errors[] = "You need to provide a valid email";
    }
    
    //check length of password - in registration
    if(strlen($password)<=7){
      $errors[] = "Your password should be atleast 8 characters.";
    }
    ...
    if($usercount < 1){
      $errors[] = "The email doesn't exist in the database";
    }

    if(!password_verify($password, $user['Password'])){
      $errors[] = "The password does not match the one in records. Try again";
    }
  ...
  <h2 style="text-align:center;">Welcome Back</h2><hr>
   <form action="login.php" method="POST">
    <div class="form-group">
      <input type="email" name="email" id="email" value="<?php echo$email;?>" onblur="checkInput(this)">
       <label for="email">Email:</label>
    </div>
     <div class="form-group">
      <input type="password" name="password" id="password" value="<?php echo$password;?>" onblur="checkInput(this)">
   ...
   <p class="text-right">Not Registered<a href="/project/admin/registration.php">Click Here!</a></p><!--could remove -->
 </div>
 <!-- <footer>
